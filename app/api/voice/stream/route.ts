...
export async function POST(req: NextRequest) {
  ...
  const log = logger.child({ requestId, route: "voice/stream" });

  const limited = enforceRateLimit(req, "voice-stream", requestId);
  ...
  try {
    ...
    log.info("streaming transcription complete", { sessionId, segments: segments.length, languages });
    ...
    log.error("streaming transcription failed", { sessionId, error: e });
