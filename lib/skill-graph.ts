...
/**
 * Extract technical topics from conversation messages.
 * Scans message content for known technical terms and returns
 * the unique set of matched topics.
 */
export function extractTopics(messages: { content: string }[]): string[] {
...
export async function rememberSession(input: RememberSessionInput): Promise<{
  ...
  const topics = extractTopics(input.session.messages);
  const skillNamesWithTopics = Array.from(new Set<string>([...skillNames, ...topics]));
  const metrics = input.session.metrics;
  const sessionId = `session:${g.revision + 1}:${slug(scenarioTitle)}`;

  const skillIds: string[] = [];
  for (const name of skillNamesWithTopics) {
    ...
    const isTopic = topics.includes(name);
    g.skills[id] = {
      id,
      name,
      category: isTopic ? "technical" : (tags[0] ?? "communication"),
...
export function serializeSessionForCognee(learnerId: string, session: PracticeSessionNode, g: SkillGraph): string {
  ...
  for (const t of session.topics ?? []) {
    lines.push(`${who} discussed the topic "${t}" during this session.`);
  }
...
export function buildDemoSkillGraph(learnerId: string, name: string | null = null): SkillGraph {
  ...
  for (const d of [...DEMO_SESSIONS].sort((a, b) => b.daysAgo - a.daysAgo)) {
    ...
    const sessionTopics = [
      ...d.topics,
      ...extractTopics([{ content: `${scenarioTitle} ${d.summary} ${d.extraSkills.map((s) => s.name).join(" ")}` }]),
    ];
    const sessionId = `session:${g.revision + 1}:${slug(scenarioTitle)}`;
    const skillIds: string[] = [];
